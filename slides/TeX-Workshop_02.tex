...
\item[\texttt{\kern-.95ex*\{n\}\{\meta{kürz}\}}] setzt $n$ mal \meta{kürz}, z.\,B. \verb/*{3}{p{4cm}|}/
\end{description}
\end{frame}
...
\begin{tabular}{l|c||r|p{2cm}|c|}
  ...
  links & mitte &  & eine lange vierte Spalte, die umgebrochen wird\\\hline
  & & & &
\end{tabular}
\end{LTXexample}
\end{frame}
...
\subsection[tabularray]{unterschiedliche Spaltenbreiten}
%\nocite{tabularray}

\begin{frame}[fragile]{unterschiedliche Spaltenbreite}
\begin{itemize}
\item Paket \pkg{tabularray} bietet vielfältige Gestaltungsmöglichkeiten für Tabellen.
\item klassiche Nutzung: \\
\verb|\begin{tblr}{|\meta{Spaltendefinitionen}\verb|}|\\
~~~~\meta{Tablleninhalt}\\
\verb|\end{tblr}|
\item erweiterte Eingabemöglichkeiten:\\
\verb|\begin{tblr}{colspec={|\meta{Spaltendef.}\verb|}, |\meta{weitere Optionen}\verb|}|\\
~~~~\meta{Tablleninhalt}\\
\verb|\end{tblr}|
\end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Tabelle mit \pkg{tabularray}}
\begin{LTXexample}[pos=b,preset={\small}]
\begin{tblr}{
...
\begin{frame}[fragile]{unterschiedliche Spaltenbreiten}
Neuer Spaltentyp:
\begin{description}
\item[\texttt{X[\meta{Faktor},\meta{Typ}]}] (linksbündige) Spalte mit variabler Breite
\end{description}
\vfill

Zur Verfügung stehende Breite wird gleichmäßig auf alle \verb|X|-Spalten verteilt:
...
\begin{frame}[fragile]{Umbrüche in Zellen}
Zeilen können mit \verb|{ \\ }| umgebrochen werden, wenn der Zellinhalt eingeklammert ist:
...
\begin{frame}[fragile]{vertikale Positionierung}
Zeilentypen \texttt{h}, \texttt{m} und \texttt{b\{\meta{Höhe}\}} richten Inhalt an Kopf, Mitte bzw. Fuß der Zeile aus.
...
\begin{frame}[fragile]{Zellen über mehrere Spalten/Zeilen}
 \verb|\SetCell[r=|\meta{Zeilen}\verb|,c=|\meta{Spalten}\verb|]{|\meta{Ausrichtung}\verb|}| vergrößert aktuelle Zelle
 \vfill
...
\begin{frame}[fragile]{farbige Tabellen}
...
\end{tblr}
\end{LTXexample}
Neben \pkg{tabularray} muss das Paket \pkg{xcolor} geladen sein.
\end{frame}


\begin{frame}[fragile]{Mathe in Tabellen}
\begin{description}
\item[\texttt{X[\$\textcolor{gray}{/}\$\$]}] startet inline-/display-Mathemodus automatisch in der ganzen Spalte
\item[S] wird automatisch am Dezimaltrennzeichen ausgerichtet \\ benötigt \verb|\UseTblrLibrary{siunitx}| \\ Text muss mit \verb|guard| gekennzeichnet sein
\end{description}
...
\begin{tblr}{llS[table-format=3.2]} \toprule
  \SetCell[c=2]{c} Artikel & & \\ \cmidrule[r]{1-2}
...
\begin{frame}{Nützlich beim Umgang mit Tabellen …}
\begin{itemize}
...
\item \pkg{tabularray}-Libraries binden bestehende Pakete in \texttt{tblr}-Syntax ein\\
Laden mit \texttt{\textbackslash UseTblrLibrary\{\meta{library}\}}  (siehe Dokumentation)
\begin{description}
\item[\texttt{amsmath}] Tabellen-Funktionen z.\,B. in Matrizen benutzen
\item[\texttt{booktabs}] \emph{schöne} Tabellen setzen
\item[\texttt{diagbox}] ersten Zelle diagonal Teilen
\item[\texttt{siunitx}] Daten in Tabellen am Dezimalpunkt ausrichten
\end{description}
\item \texttt{longtblr}-Umgebung erlaubt Tabellen mit Fußnoten und Seitenumbrüchen 

\vfill
\item Praktisches Online-Tool: \href{https://www.tablesgenerator.com/}{Tables Generator}\\
...
\nocite{mathmode, dante:mathe, amsmath, tabularray, booktabs, dante:tabellen, dante:wissarbeit, pgfplots} %dante:koma, 
\begin{frame}[allowframebreaks]{Weiterführende Literatur}
\printbibliography
\end{frame}


\begin{frame}<handout:1>{Lehrevaluation}
